...
- [x] **What the shop stores about its customers.** Decided 23 September 2026. The shop keeps a customer's **phone number and name**, and their **WhatsApp conversations**, so the ordering agent can see previous orders and offer "the usual". A `customers` table keyed by phone number (the WhatsApp identity) holds the name; conversations and orders link to it. Stored in Supabase under RLS: staff with the relevant key and the agent's own narrow role can read them, and nothing else can. No address, date of birth or other personal detail is kept unless a later decision adds it (delivery addresses are the fulfilment card's to propose). This also unblocks WhatsApp receipt sharing, which has waited on this since the line above.
...
**Customer data (decided, §2):** the shop stores each customer's phone number and name, and their WhatsApp conversations for order history, in a `customers` table keyed by phone under RLS.
